...
public class Map extends JPanel {
    ...
    private static final String MSG_WIN_HUMAN = "EXCELLENT!";
    ...
    Map() {
        ...
        // MouseListner слушитель мышки
        addMouseListener(new MouseAdapter() {
            @Override
            public void mouseReleased(MouseEvent e) {
                super.mouseReleased(e); // по отжатие кнопки
                update(e);
            }
        });
        initialized = false;
    ...
    private void update(MouseEvent e) {
        ...
        if (!initialized) return;   // вычислениее координат мышки
    ...
    private void render(Graphics g) {
        if (!initialized) return;       // получаем сетку
        int width = getWidth();
        int height = getHeight();       // полоски
    ...
    @Override   // компонент графики
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        render(g);  // мы будем рендрить
    }
